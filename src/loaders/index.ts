import { ConfigModule, StoreService, MedusaContainer } from "@medusajs/medusa";
import { runMigration } from "contentful-migration";
import { productMigration } from "./contentful-migrations/product";
import { productVariantMigration } from "./contentful-migrations/product-variant";
import { productCollectionMigration } from "./contentful-migrations/product-collection";
import { productTypeMigration } from "./contentful-migrations/product-type";
import { regionMigration } from "./contentful-migrations/region";
...
export default async (
  ...
  if (!contentfulPlugin) {
    console.log("Didn't find Contentful plugin. Aborting migration...");
    return;
  }
  ...
  await Promise.all(
    migrationFunctions.map(async (migrationFunction) => {
      console.info(`Migrating ${migrationFunction.name} component...`);
      try {
        await runMigration({
          ...options,
          migrationFunction: migrationFunction.function,
        });
        console.info(`Finished migrating ${migrationFunction.name} component`);
      } catch (e) {
        if (
          typeof e === "object" &&
          "errors" in e &&
          ...
          console.info(
            `${migrationFunction.name} already exists. Skipping its migration.`
          );
        } else {
          throw new Error(e);
        }
